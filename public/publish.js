import { initializeApp }   from "https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js";
    ...
    function setSlugStatus(type, msg) {
      ...
      if (!type || !msg) { slugStatus.innerHTML = ''; return; }
      const map = { avail:'ss-avail', taken:'ss-taken', check:'ss-check' };
      const icon = type==='avail' ? 'fas fa-check-circle' : type==='taken' ? 'fas fa-times-circle' : 'fas fa-spinner fa-spin';
    ...
    document.getElementById('checkSlugBtn').addEventListener('click', () => {
    ...
    async function checkSlug(slug) {
      ...
      try {
        ...
        if (takenByOther) {
          setSlugStatus('taken', `"${slug}" is already taken`);
        } else {
          setSlugStatus('avail', `"${slug}" is available!`);
        }
      } catch { setSlugStatus('', ''); }
    }
    ...
    function showLiveCard(slug) {
      ...
      document.getElementById('shareLinkedIn').href = `https://www.linkedin.com/shareArticle?url=${enc}`;
      document.getElementById('shareWhatsapp').href = `https://wa.me/?text=${encodeURIComponent('Check out my portfolio: ' + url)}`;
      document.getElementById('shareEmail').href    = `mailto:?subject=My Portfolio&body=Check out my portfolio: ${url}`;
    ...
    document.getElementById('publishBtn').addEventListener('click', async () => {
      ...
      if (snap.docs.some(d => d.id !== currentUser.uid)) {
        showToast(`"${slug}" is already taken. Choose another.`, 'error'); return;
      }
      ...
      try {
        ...
        const visibility = document.querySelector('.vis-opt.active')?.dataset.vis || 'public';
    ...
    onAuthStateChanged(auth, async user => {
      ...
      skillsCount = sk.size; projCount = pr.size; eduCount = ed.size;
      ...
      // Template
      const tpl = localStorage.getItem('finalTemplate');
      checkStatus.template = !!tpl;
      if (tpl) document.getElementById('tplNameEl').textContent = tpl;
